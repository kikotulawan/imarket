<template>
 <div class="container vh-100 w-100 px-3 pb-10 d-flex flex-column align-items-start overflow-y-auto">
  ...
  <div class="animate__animated animate__slideInLeft">
   <p class="text-dark text-h4 fw-bold mb-0 pb-0 lh-1">ORDER ACCEPTED</p>
   <p class="text-secondary text-subtitle-2 mb-10">The user is now waiting in your delivery</p>
  </div>
  <div class="animate__animated animate__slideInUp w-100">
   ...
   <hr />
   <v-btn class="text-white mt-1" type="button" block depressed raised elevation="3" style="background: #0d6efd" @click="getDirection"> GET DIRECTIONS </v-btn>
   <v-btn class="text-white mt-3" type="button" block depressed raised elevation="3" style="background: #349f6e" @click="completeOrder"> COMPLETE ORDER </v-btn>
  </div>
 </div>
</template>
<script>
 ...
 export default {
  ...
  methods: {
   ...
   getDirection() {
    let href = `https://www.google.com/maps/dir/${this.center.lat},${this.center.lng}/${this.order.buyer.user.address.lat},${this.order.buyer.user.address.lng}/@${this.order.buyer.user.address.lat},${this.order.buyer.user.address.lng}?hl=en-US`;
    window.location.href = href;
   },
   async completeOrder() {
    const res = await this.$store.dispatch("auth/completeOrder", this.$route.params.id);
    res.status == 200 ? (this.$toast.success("Order delivered!"), this.$router.push({ path: `/order-completed/${this.$route.params.id}` })) : this.$toast.error("Something went wrong, try again.");
   },
   async orderInfo() {
