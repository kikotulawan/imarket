<template>
 <div class="container vh-100 w-100 px-3 pb-10 d-flex flex-column align-items-start overflow-y-auto">
  ...
  <div class="animate__animated animate__slideInLeft">
   <p class="text-dark text-h4 fw-bold mb-0 pb-0 lh-1">ORDER DETAILS</p>
   <p class="text-secondary text-subtitle-2 mb-10">Click on confirm to notify the user that one of our rider is on their way</p>
  </div>
  <div class="animate__animated animate__slideInUp w-100">
   ...
   <hr />
   <v-btn class="text-white mt-1" type="button" block depressed raised elevation="3" style="background: #349f6e" @click="acceptOrder"> ACCEPT ORDER </v-btn>
  </div>
 </div>
</template>
<script>
 export default {
  ...
  methods: {
   ...
   async acceptOrder() {
    const res = await this.$store.dispatch("auth/acceptOrder", this.$route.params.id);
    res.status == 200 ? (this.$toast.success("Order accepted"), this.$router.push({ path: `/order-accepted/${this.$route.params.id}` })) : this.$toast.error("Something went wrong, try again.");
   },
   async orderInfo() {
