<template>
 <div>
  ...
  <v-container class="">
   <search-product />
   <v-row v-if="productInfo.product_info" class="mb-10 mt-8">
    <v-col cols="12" sm="12" md="7" lg="6">
     <v-img max-height="46 0" :src="`https://imarketbe.capstone-group.online/images/products/${productInfo.product_info.image}`"></v-img>
    </v-col>
    <v-col cols="12" sm="12" md="5" lg="5">
     <h1 class="ml-4 mt-1">{{ productInfo.product_info.name }}</h1>
     <router-link class="ml-4 mb-0 text-decoration-none" :to="`/imarket/store/${productInfo.store.id}`">{{ productInfo.store.name }}</router-link>
     <h4 class="ml-4 mt-3 font-weight-regular text-justify">{{ productInfo.product_info.description }}</h4>
     <v-layout class="ml-3 mt-4" d-flex align-center>
      <v-rating class="" empty-icon="mdi-star-outline" full-icon="mdi-star" half-icon="mdi-star-half-full" half-increments hover length="5" background-color="primary lighten-2" color="primary" :value="productInfo.rating_avg_rating" size="25" readonly></v-rating>
      <p class="mb-0 ml-2 black--text">
       {{ productInfo.rating_avg_rating && productInfo.rating_avg_rating.toFixed(1) }}
      </p>
     </v-layout>
     <p class="mb-0 ml-4 mt-2">{{ productInfo.rating_count }} Rating{{ productInfo.rating_count > 1 ? "s" : "" }}</p>
     <h1 class="mt-4 ml-4 orange--text darken-2 font-weight-regular">{{ formatCurrency(productInfo.product_info.price) }}</h1>
     <v-layout class="ml-4 mt-6" align-center>
      <p class="font-weight-bold mb-0 mr-2">QTY:</p>
      <v-col lg="3" md="3" sm="4" cols="5">
       <v-text-field v-model="data.qty" type="number" min="1" :max="productInfo.product_info.quantity" outlined dense hide-details="auto"></v-text-field>
      </v-col>
     </v-layout>
     <v-btn @click="addToCart(productInfo)" :loading="btnLoading" depressed large class="ml-4 btn-glow-blue" color="blue darken-2" dark>
     ...
     <v-layout d-flex class="mt-5" v-for="(rating, i) in productSelected.rating" :key="i">
      ...
      <v-layout column>
       <v-layout d-flex align-center class="mb-0">
        <v-rating class="ml-8" empty-icon="mdi-star-outline" full-icon="mdi-star" half-icon="mdi-star-half-full" half-increments hover length="5" :value="parseFloat(rating.rating)" size="20" background-color="primary lighten-2" color="primary" readonly></v-rating>
      ...
      <v-col cols="12" sm="6" md="10" lg="10" v-for="(product, i) in relatedCategoryProduct" :key="i">
       <v-hover :key="i" v-slot="{ hover }" class="cursor-hover">
        <v-card link :to="`/imarket/product/${product.id}`" class="mx-2 mb-3" :elevation="hover ? 2 : 0" :outlined="hover ? true : false">
         <v-img :src="`https://imarketbe.capstone-group.online/images/products/${product.product_info.image}`" contain></v-img>
         <v-card-text class="position-relative pt-2">
          <v-btn @click.prevent="addToCart(product)" :loading="isCartLoading && productId == product.id" absolute :color="isAddedSuccess && productId == product.id ? 'green' : 'orange darken-2'" class="white--text" fab small right top depressed>
          ...
          <v-layout d-flex align-center>
           <v-rating class="mt-n4" empty-icon="mdi-star-outline" full-icon="mdi-star" half-icon="mdi-star-half-full" half-increments hover length="5" :value="product.rating_avg_rating" size="18" background-color="primary lighten-2" color="primary" readonly></v-rating>
     ...
     <v-hover :key="i" v-slot="{ hover }" class="cursor-hover">
      <v-card link :to="`/imarket/product/${product.id}`" class="mx-2 mb-3" :elevation="hover ? 2 : 0" :outlined="hover ? true : false">
       ...
       <v-card-text class="position-relative pt-2">
        ...
        <v-card-subtitle class="pt-0 pl-1"> {{ product.store.name }} </v-card-subtitle>
        <v-layout d-flex align-center>
         <v-rating class="mt-n4" empty-icon="mdi-star-outline" full-icon="mdi-star" half-icon="mdi-star-half-full" half-increments background-color="primary lighten-2" color="primary" hover length="5" :value="product.rating_avg_rating" size="18" readonly></v-rating>
         <p class="mb-0 mt-n3 ml-2 black--text">
          {{ product.rating_avg_rating && product.rating_avg_rating.toFixed(1) }}
         </p>
        </v-layout>
