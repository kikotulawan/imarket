  ...
  <v-container class="">
   <v-row justify="center">
    <v-col cols="12" sm="11" md="5" lg="4">
     <v-card rounded="lg" elevation="0" class="pt-6 pb-5">
      <p class="mb-0 text-h5 font-weight-bold">My Orders</p>
      <div class="pr-5" v-if="!orders || (orders.data && orders.data.length == 0)">
       <p class="mt-5 w-75">You currently have no history of orders on your account. Continue shopping and make some orders.</p>
       <v-btn class="" color="primary" depressed link to="/">Go Shopping</v-btn>
      </div>
      <p v-else class="">Click any of the list below to view details of your order</p>
      <v-list>
    ...
    <v-col cols="12" sm="12" md="7" lg="8">
     <v-card elevation="0" class="overflow-scroll" max-height="490">
      <p class="mb-0 text-h5 font-weight-bold">Order Content</p>
      <p class="">Here are the content of your orders.</p>
      <div class="" v-if="Object.keys(orderContent).length == 0 || orderContent.length == 0">
       <p class="mt-5 w-75">Select an order to view its content</p>
      </div>
      <v-list>
