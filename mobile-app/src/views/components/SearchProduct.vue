<template>
 <div>
  ...
  <v-layout column class="mb-8 lh-small">
   ...
   <v-layout class="d-flex flex-column" v-else>
    <p class="mt-5 text-h5 font-poppins font-weight-bold mb-0">Please log-in to your account to make purchases.</p>
    <v-btn class="mt-4 text-light" color="black"> <router-link to="/login" class="text-decoration-none"> Login Account</router-link> </v-btn>
   </v-layout>
  ...
  <v-layout>
   <v-text-field prepend-inner-icon="mdi-magnify" @keypress.enter="searchProductOrRecipe" append-outer-icon="mdi-cog" @click:append-outer="searchCustomDialog = true" v-model="search" class="rounded-lg" label="What would you like to eat?" required filled rounded clearable color="orange darken-2"></v-text-field>
  </v-layout>
