 ...
 <div class="mb-12">
  ...
  <v-data-table :search="search" :headers="headers" :items="products" :items-per-page="5" class="elevation-0">
   <template v-slot:item.product_info.image="{ item }">
    <v-img class="mt-2 mb-2" max-width="80" :src="`https://imarketbe.capstone-group.online/images/products/${item.product_info.image}`"></v-img>
   </template>
   ...
   <template v-slot:item.actions="{ item }">
    <v-layout d-flex>
     ...
     <v-btn @click="setDeleteProduct(item)" color="red" depressed plain>Delete</v-btn>
    </v-layout>
   </template>
  </v-data-table>
  <v-dialog v-model="dialogNewProduct" max-width="530">
   <v-card class="pt-5">
    <v-form @submit.prevent="saveProduct" class="pl-5 pr-5" lazy-validation v-model="validProduct" ref="newProduct">
     ...
     <v-file-input :rules="required" @change="uploadImage" accept="image/png, image/jpeg, image/bmp" outlined prepend-icon="" prepend-inner-icon="mdi-file-image" show-size counter label="Product Image"></v-file-input>
     ...
     <v-text-field class="mt-3" hide-details="auto" prepend-inner-icon="mdi-card-bulleted-outline" v-model="data.description" :rules="required" label="Description" required outlined></v-text-field>
     <v-layout d-flex>
  ...
  <v-dialog v-model="dialogUpdateProduct" max-width="530">
   <v-card class="pt-5">
    <v-form @submit.prevent="updateProduct" class="pl-5 pr-5" lazy-validation v-model="validProduct" ref="updateProduct">
     ...
     <h4 class="font-weight-light mb-5">All fields are required</h4>
     <v-file-input @change="uploadImage" accept="image/png, image/jpeg, image/bmp" outlined prepend-icon="" prepend-inner-icon="mdi-file-image" show-size counter label="Product Image"></v-file-input>
     ...
     <v-text-field class="mt-3" hide-details="auto" prepend-inner-icon="mdi-card-bulleted-outline" v-model="product.description" :rules="required" label="Description" required outlined></v-text-field>
     <v-layout d-flex>
      ...
      <v-text-field type="number" class="mt-3" hide-details="auto" prepend-inner-icon="mdi-numeric" v-model="product.quantity" :rules="required" label="Quantity" required outlined></v-text-field>
     </v-layout>
