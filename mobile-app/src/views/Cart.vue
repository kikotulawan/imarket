 ...
 <div>
  ...
  <v-container class="">
   <v-row>
    <v-col cols="12" md="7" lg="7">
     <v-card elevation="0" class="overflow-scroll" max-height="490">
      <p class="mb-0 text-h5 font-weight-bold">My Cart</p>
      <p class="">Here are your added to cart products.</p>
      <div class="" v-if="Object.keys(cart).length == 0 || cart.cart_info.length == 0">
       <p class="mt-5 w-75">There are no items in your cart. Continue shopping and start adding items to your cart now.</p>
      </div>
      <v-list>
       <div v-for="(product, i) in cart.cart_info" :key="i">
        <v-list-item>
         <v-list-item-avatar size="50" tile>
          <v-img class="rounded-lg" :src="'https://imarketbe.capstone-group.online/images/products/' + product.product.product_info.image"></v-img>
         </v-list-item-avatar>
    ...
    <v-col cols="12" md="5" lg="5">
     <v-card elevation="0" class="pb-8 pt-8">
      ...
      <v-btn :disabled="Object.keys(cart).length == 0 || cart.cart_info.length == 0 || user.address.formatted_address == null" @click.prevent="checkout" class="" color="orange darken-2 white--text" block depressed :loading="btnLoading">CHECKOUT</v-btn>
     </v-card>
    </v-col>
   </v-row>
  </v-container>
  <v-container>
   ...
   <v-row dense no-gutters class="">
    <v-col cols="12" sm="4" md="3" lg="3" v-for="(product, i) in latest_products" :key="i">
     <v-hover :key="i" v-slot="{ hover }" class="cursor-hover">
      <v-card link :to="`/imarket/product/${product.id}`" class="mx-2 mb-3" :elevation="hover ? 2 : 0" :outlined="hover ? true : false">
       <v-img :src="`https://imarketbe.capstone-group.online/images/products/${product.product_info.image}.png`" contain></v-img>
       <v-card-text class="position-relative pt-2">
        ...
        <v-layout d-flex align-center>
         <v-rating class="mt-n4" empty-icon="mdi-star-outline" full-icon="mdi-star" half-icon="mdi-star-half-full" half-increments hover length="5" :value="product.rating_avg_rating" background-color="primary lighten-2" color="primary" size="18" readonly></v-rating>
    ...
    <v-layout justify-center class="mt-5 mb-5">
     <v-btn link to="/" large depressed color="blue darken-2" dark :loading="isLoading">MORE PRODUCTS</v-btn>
    </v-layout>
   </v-row>
  </v-container>
