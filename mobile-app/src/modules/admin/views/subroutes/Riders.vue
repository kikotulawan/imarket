<template>
 <v-container :class="{ 'pa-0': $vuetify.breakpoint.md }" :fluid="isFluid">
  <div class="mt-15">
   <p class="text-h5 font-weight-bold custom-primary-color mt-8 mb-0">Riders Management</p>
   <p>Shown below are the all the riders for the delivery</p>
  </div>
    ...
    <v-card elevation="2" color="blue darken-1" class="pa-3" rounded="lg" dark>
     <div class="d-flex flex-no-wrap justify-space-between align-end mb-2">
      <div>
       ...
       <v-card-title class="text-h5 font-grotesk font-weight-bold mb-0 pa-0 pl-4 pb-2 pt-4">Active <br />Riders</v-card-title>
      </div>
      <div>
    ...
    <v-card elevation="2" color="red lighten-1" class="pa-3" rounded="lg" dark>
     <div class="d-flex flex-no-wrap justify-space-between align-end mb-2">
      <div>
       ...
       <v-card-title class="text-h5 font-grotesk font-weight-bold mb-0 pa-0 pl-4 pb-2 pt-4">Archived <br />Riders</v-card-title>
      </div>
      <div>
    ...
    <v-card elevation="2" color="green " class="pa-3" rounded="lg" dark>
     <div class="d-flex flex-no-wrap justify-space-between align-end mb-2">
      <div>
       ...
       <v-card-title class="text-h5 font-grotesk font-weight-bold mb-0 pa-0 pl-4 pb-2 pt-4">New <br />Riders</v-card-title>
      </div>
      <div>
...
<script>
 ...
 export default {
  ...
  methods: {
   ...
   async getUsers() {
    await this.$store.dispatch("adminRider/getRiders");
   },
   async getArchivedUsers() {
    await this.$store.dispatch("adminRider/getArchivedRiders");
   },
   async archiveUser() {
    const { status, data } = await this.$store.dispatch("adminRider/archiveRider", this.deleteData);
    this.toastData(status, data);
    this.archiveDialog = false;
    this.deleteData = null;
   },
   async deleteUser() {
    const { status, data } = await this.$store.dispatch("adminRider/deleteRider", this.deleteData);
    this.toastData(status, data);
    this.deleteDialog = false;
    this.deleteData = null;
   },
   async restoreUser() {
    const { status, data } = await this.$store.dispatch("adminRider/restoreRider", this.restoreData);
    this.toastData(status, data);
