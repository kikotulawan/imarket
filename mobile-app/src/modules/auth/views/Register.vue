  ...
  <v-container class="">
   <v-row justify="center" class="h-100">
    <v-col cols="12" sm="11" md="6" lg="6">
     ...
     <v-btn v-animate-css="{ classes: 'fadeInUp', delay: 850 }" class="mt-4 text-light" color="black"> <router-link to="/" class="text-decoration-none"> Go back</router-link> </v-btn>
       ...
       <v-form ref="form" lazy-validation v-model="valid" @submit.prevent="register">
        <v-stepper-content step="1">
         <div class="mt-2">
          ...
          <v-text-field prepend-inner-icon="mdi-phone-in-talk" class="mt-3" outlined hide-details="auto" v-model="data.contact_number" :rules="minChar" label="Contact Number" type="text" required></v-text-field>
        ...
        <v-stepper-content step="2">
         <div class="mt-2">
          ...
          <v-text-field class="mt-3" prepend-inner-icon="mdi-key" type="password" v-model="data.password" hide-details="auto" :rules="required" label="Password" required outlined></v-text-field>
          <v-textarea class="mt-3" rows="2" prepend-inner-icon="mdi-map-marker" v-model="data.address" :rules="required" @click.prevent="addressDialog = true" auto-grow hide-details="auto" label="Address" readonly required outlined></v-textarea>
          <v-menu ref="menu" v-model="menu" :close-on-content-click="false" :return-value.sync="data.birthday" transition="scale-transition" offset-y min-width="auto">
           <template v-slot:activator="{ on, attrs }">
            <v-text-field class="mt-4" hide-details="auto" outlined v-model="data.birthday" :rules="required" label="Date of Birth" prepend-inner-icon="mdi-calendar" readonly v-bind="attrs" v-on="on"></v-text-field>
           </template>
