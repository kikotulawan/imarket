<template>
 <v-container :class="{ 'pa-0': $vuetify.breakpoint.md }" :fluid="isFluid">
  <div class="mt-15">
   <p class="text-h5 font-weight-bold custom-primary-color mt-8 mb-0">Partners</p>
   <p>Shown below are the all the partners in the system</p>
  </div>
    ...
    <v-card elevation="2" color="blue darken-1" class="pa-3" rounded="lg" dark>
     <div class="d-flex flex-no-wrap justify-space-between align-end mb-2">
      <div>
       ...
       <v-card-title class="text-h5 font-grotesk font-weight-bold mb-0 pa-0 pl-4 pb-2 pt-4">Active <br />Partners</v-card-title>
      </div>
      <div>
    ...
    <v-card elevation="2" color="red lighten-1" class="pa-3" rounded="lg" dark>
     <div class="d-flex flex-no-wrap justify-space-between align-end mb-2">
      <div>
       ...
       <v-card-title class="text-h5 font-grotesk font-weight-bold mb-0 pa-0 pl-4 pb-2 pt-4">Archived <br />Partners</v-card-title>
      </div>
      <div>
    ...
    <v-card elevation="2" color="green " class="pa-3" rounded="lg" dark>
     <div class="d-flex flex-no-wrap justify-space-between align-end mb-2">
      <div>
       ...
       <v-card-title class="text-h5 font-grotesk font-weight-bold mb-0 pa-0 pl-4 pb-2 pt-4">New <br />Partners</v-card-title>
      </div>
      <div>
    ...
    <v-data-table :headers="headers" :items="users" :search="search" :loading="isLoading" :loading-text="'Retrieving users data. Please wait ...'">
     <template v-slot:item.profile="{ item }">
      <v-avatar class="ma-0" size="45" color="primary">
       <img class="cursor-pointer" v-if="item.info.profile_img" :src="`https://imarketbe.capstone-group.online/images/profiles/${item.info.profile_img}`" />
  ...
  <v-row class="mb-5">
   <v-col>
    ...
    <v-data-table :headers="archivedHeaders" :items="archivedUsers" :search="searchArchived" :loading="isLoading" :loading-text="'Retrieving users data. Please wait ...'">
     <template v-slot:item.profile="{ item }">
      <v-avatar class="ma-0" size="45" color="primary">
       <img class="cursor-pointer" @click.prevent="showProfile(item.info.profile_img)" v-if="item.info.profile_img" :src="`https://imarketbe.capstone-group.online/images/profiles/${item.info.profile_img}`" />
       <p v-else class="white--text font-weight-bold mb-0">{{ item.info.first_name[0] }}{{ item.info.last_name[0] }}</p>
      </v-avatar>
     </template>
     <template v-slot:item.info.full_name="{ item }">
  ...
  <v-dialog v-model="userInfoDialog" max-width="460">
   <v-card class="pt-2 pl-4 pr-4 pb-2">
     ...
     <v-card-text>
      <v-layout justify-center>
       <v-avatar class="ma-0 mt-2" size="150" color="primary">
        <img class="cursor-pointer" @click.prevent="showProfile(userSelectedData.info.profile_img)" v-if="userSelectedData.info.profile_img" :src="`https://imarketbe.capstone-group.online/images/profiles/${userSelectedData.info.profile_img}`" />
    ...
    <v-layout class="pl-3 pr-3">
     ...
     <v-col>
      <p
       ...
       {{ userSelectedData.info.birthday }}</p
      >
     </v-col>
     <v-col>
      <p
       ><span class="font-weight-bold">Gender:</span><br />
       {{ userSelectedData.info.gender }}</p
      >
     </v-col>
    </v-layout>
    <v-layout class="pl-3 pr-4">
     <v-col>
      <p
       ><span class="font-weight-bold">Addresss:</span><br />
       {{ userSelectedData.address && userSelectedData.address.formatted_address ? userSelectedData.address.formatted_address : "Address has not been set yet." }}</p
      >
     </v-col>
     <v-col>
      <p
       ><span class="font-weight-bold">Created On:</span><br />
       {{ userSelectedData.created_at }}</p
      >
     </v-col>
    </v-layout>
    <v-card-actions>
...
<script>
 ...
 export default {
  ...
  methods: {
   ...
   async getUsers() {
    await this.$store.dispatch("adminRider/getPartners");
   },
   async getArchivedUsers() {
    await this.$store.dispatch("adminRider/getArchivedPartners");
   },
   async archiveUser() {
    const { status, data } = await this.$store.dispatch("adminRider/archivePartner", this.deleteData);
    this.toastData(status, data);
    this.archiveDialog = false;
    this.deleteData = null;
   },
   async deleteUser() {
    const { status, data } = await this.$store.dispatch("adminRider/deletePartner", this.deleteData);
    this.toastData(status, data);
    this.deleteDialog = false;
    this.deleteData = null;
   },
   async restoreUser() {
    const { status, data } = await this.$store.dispatch("adminRider/restorePartner", this.restoreData);
    this.toastData(status, data);
  ...
  computed: {
   ...mapState("adminRider", ["users", "archivedUsers", "newUsersCount"]),
   isFluid() {
