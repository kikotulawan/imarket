    ...
    <v-data-table class="" :headers="headers" :items="stores" :search="search" :loading="isLoading" :loading-text="'Retrieving stores data. Please wait ...'">
     <template v-slot:item.logo="{ item }">
      <v-avatar class="ma-0" size="45" color="primary">
       <img class="cursor-pointer" v-if="item.logo" :src="`https://imarketbe.capstone-group.online/images/logos/${item.logo}`" />
     ...
     <template v-slot:item.actions="{ item }">
      <v-layout>
       <v-btn
        ...
        >Approve</v-btn
       >
       <v-btn link v-if="item.status != 'For Approval'" :to="`/imarket/store/${item.id}`" small text target="_blank" color="green darken-1">View</v-btn>
       <v-btn
        @click="
         deleteData = item;
         archiveDialog = true;
  ...
  <v-row class="mb-5">
   <v-col>
    ...
    <v-data-table :headers="archivedHeaders" :items="archivedStores" :search="searchArchived" :loading="isLoading" :loading-text="'Retrieving users data. Please wait ...'">
     <template v-slot:item.logo="{ item }">
      <v-avatar class="ma-0" size="45" color="primary">
       <img class="cursor-pointer" @click.prevent="showProfile(item.logo)" v-if="item.logo" :src="`https://imarketbe.capstone-group.online/images/logos/${item.logo}`" />
       <p v-else class="white--text font-weight-bold mb-0">{{ item.name[0] }}</p>
      </v-avatar>
     </template>
     <template v-slot:item.description="{ item }">
