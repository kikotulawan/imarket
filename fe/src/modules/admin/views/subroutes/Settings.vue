 ...
 <div>
     ...
     <v-form ref="form" v-model="valid" @submit.prevent="update" lazy-validation>
      <v-row dense class="mt-3">
       <v-col cols="12" sm="12" md="6" lg="6">
        <v-text-field prepend-inner-icon="mdi-card-text-outline" class="" hide-details="auto" :rules="required" label="First Name" :loading="isLoading" v-model="data.first_name" required outlined></v-text-field>
       </v-col>
       <v-col cols="11" sm="12" md="6" lg="6">
        <v-text-field prepend-inner-icon="mdi-card-text-outline" class="" hide-details="auto" label="Middle Name" :loading="isLoading" v-model="data.middle_name" required outlined></v-text-field>
       </v-col>
       <v-col cols="11" sm="12" md="6" lg="6">
        <v-text-field prepend-inner-icon="mdi-card-text-outline" class="" hide-details="auto" :rules="required" label="Last Name" :loading="isLoading" v-model="data.last_name" required outlined></v-text-field>
       </v-col>
       <v-col cols="11" sm="12" md="6" lg="6">
        <v-text-field prepend-inner-icon="mdi-map-marker" class="" hide-details="auto" :rules="required" label="Address" :loading="isLoading" v-model="data.address" readonly @click="showAddressPicker" required outlined></v-text-field>
       </v-col>
       <v-col cols="11" sm="12" md="6" lg="6">
       ...
       <v-col cols="11" sm="12" md="6" lg="6">
        <v-text-field prepend-inner-icon="mdi-phone" class="" hide-details="auto" :rules="required" label="Contact Number" :loading="isLoading" v-model="data.contact_number" outlined></v-text-field>
       </v-col>
       <v-col cols="11" sm="12" md="6" lg="6">
        <v-menu ref="menu" v-model="menu" :close-on-content-click="false" transition="scale-transition" offset-y min-width="auto">
         <template v-slot:activator="{ on, attrs }">
          <v-text-field v-model="data.birthday" label="Date of Birth" prepend-inner-icon="mdi-calendar" readonly v-bind="attrs" v-on="on" :loading="isLoading" :rules="required" outlined></v-text-field>
         </template>
      ...
      <v-row class="mt-3" align="center">
       <v-avatar color="grey darken-1" size="70" class="ml-3">
        <img @click.prevent="showProfile" class="img-cover cursor-pointer" v-if="user.info.profile_img" :src="`https://imarketbe.capstone-group.online/images/profiles/${this.user.info.profile_img}`" alt="Profile Image" />
      ...
      <v-row class="mt-3">
       <v-col cols="12" sm="12" md="6" lg="6">
        <v-text-field prepend-inner-icon="mdi-email" class="" hide-details="auto" :rules="required" label="Email Address" :loading="isLoading" v-model="data.email" required outlined></v-text-field>
       </v-col>
      ...
      <v-dialog v-model="dialog" persistent max-width="520">
       ...
       <v-card>
        ...
        <v-card-text>
         ...
         <v-text-field type="password" prepend-inner-icon="mdi-key" class="mt-3" hide-details="auto" v-model="data.current_password" :rules="required" label="Current Password" required outlined></v-text-field>
        </v-card-text>
  ...
  <v-dialog v-model="addressDialog" max-width="620">
   <v-card class="pl-4 pr-4">
    ...
    <label class="">
     <gmap-autocomplete ref="gmapAutoComplete" class="gmap-auto-complete" @place_changed="initMarker" :options="{ fields: ['geometry', 'formatted_address', 'address_components'] }"></gmap-autocomplete>
    </label>
    <v-container>
   ...
   <v-card>
    <v-form ref="formPass" @submit.prevent="savePassword" lazy-validation>
     ...
     <v-card-text class="">
      ...
      <v-text-field type="password" append-icon="mdi-key" class="mt-3" hide-details="auto" v-model="password_data.current_password" :rules="required" label="Current Password" required outlined dense></v-text-field>
      <v-text-field type="password" append-icon="mdi-key-variant" class="mt-3" hide-details="auto" v-model="password_data.new_password" :rules="required" label="New Password" required outlined dense></v-text-field>
      <v-text-field type="password" append-icon="mdi-key-variant" class="mt-3" hide-details="auto" v-model="password_data.confirm_password" :rules="[matchingPasswords]" label="Confirm Password" required outlined dense></v-text-field>
     </v-card-text>
 ...
 export default {
  ...
  methods: {
   ...
   setInfo() {
    ...
    this.data.address = this.user.address && this.user.address.formatted_address;
