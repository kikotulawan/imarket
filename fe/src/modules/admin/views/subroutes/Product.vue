<template>
 <v-container :class="{ 'pa-0': $vuetify.breakpoint.md }" :fluid="isFluid">
  <div>
   <p class="text-h5 font-weight-bold custom-primary-color mt-16 mb-0">Products Management</p>
    ...
    <v-data-table :headers="headers" :items="products" :search="search" :loading="isLoading" :loading-text="'Retrieving stores data. Please wait ...'">
     <template v-slot:item.logo="{ item }">
      <v-avatar class="ma-0" size="50" color="primary" tile>
       <img class="cursor-pointer" v-if="item.product_info.image" :src="`https://imarketbe.capstone-group.online/images/products/${item.product_info.image}`" />
     ...
     <template v-slot:item.actions="{ item }">
      <v-layout>
       ...
       <v-btn @click="showViewDialog(item)" small text color="green darken-1">View</v-btn>
       <v-btn
        @click="
         deleteData = item;
         archiveDialog = true;
  ...
  <v-dialog v-model="dialogUpdateProduct" max-width="530">
   <v-card class="pt-5">
    <v-form class="pl-5 pr-5" v-model="validProduct" lazy-validation ref="updateProduct">
     <h4 class="font-weight-bold text-h5 mb-3">Product Details</h4>
     <v-text-field readonly hide-details="auto" prepend-inner-icon="mdi-package" v-model="product.name" :rules="required" label="Name" required outlined></v-text-field>
     <v-textarea class="mt-3" readonly hide-details="auto" prepend-inner-icon="mdi-card-bulleted-outline" v-model="product.description" :rules="required" label="Description" required outlined></v-textarea>
     <v-layout d-flex>
      <v-text-field type="number" class="mt-3" readonly hide-details="auto" prepend-inner-icon="mdi-cash" v-model="product.price" :rules="required" label="Price" required outlined></v-text-field>
      <v-text-field type="number" class="mt-3" readonly hide-details="auto" prepend-inner-icon="mdi-numeric" v-model="product.quantity" :rules="required" label="Quantity" required outlined></v-text-field>
     </v-layout>
     <v-select :rules="required" class="mt-3" outlined readonly v-model="product.category" item-value="id" item-text="category" :items="categories" label="Select Category"></v-select>
     <v-card-actions class="pb-4 pt-5">
      <v-spacer></v-spacer>
      <v-btn plain color="grey darken-1" text @click="dialogUpdateProduct = false" :loading="btnLoading"> Close </v-btn>
     </v-card-actions>
    </v-form>
   </v-card>
  </v-dialog>
...
<script>
 ...
 export default {
  ...
  methods: {
   ...
   showViewDialog(item) {
    console.log(item);
    this.product = item.product_info;
