  ...
  <div class="mt-2">
   <v-layout column>
    <div class="store-preview">
     <v-img :src="`https://imarketbe.capstone-group.online/images/banners/${storeInfo.banner_image}`" class="store-preview--image"></v-img>
    ...
    <v-layout class="store-preview--content">
     <div align-center justify-center class="position-relative">
      <v-avatar color="grey" size="140">
       <img v-if="storeInfo.logo != '' && storeInfo.logo != null" :src="'https://imarketbe.capstone-group.online/images/logos/' + storeInfo.logo" alt="Store Logo" />
   ...
   <v-form ref="form" lazy-validation v-model="valid" @submit.prevent="update">
    <v-row dense>
     ...
     <v-col lg="6" md="6" sm="6" cols="12">
      <v-textarea rows="2" auto-grow class="" prepend-inner-icon="mdi-key" type="text" v-model="data.description" hide-details="auto" :rules="required" label="Store Description" required outlined></v-textarea>
     </v-col>
      ...
      <v-layout d-flex>
       <v-dialog ref="dialogWeekDaysMorning" v-model="modalWeekDaysMorning" :return-value.sync="data.week_days_opening" persistent width="400px">
        <template v-slot:activator="{ on, attrs }">
         <v-text-field class="" outlined v-model="data.week_days_opening" label="Opening Hour" prepend-inner-icon="mdi-clock-time-four-outline" readonly :rules="required" hide-details="auto" v-bind="attrs" v-on="on"></v-text-field>
        </template>
       ...
       <v-dialog ref="dialogWeekDaysAfternoon" v-model="modalWeekDaysAfternoon" :return-value.sync="data.week_days_closing" persistent width="400px">
        <template v-slot:activator="{ on, attrs }">
         <v-text-field class="" outlined v-model="data.week_days_closing" label="Closing Hour" prepend-inner-icon="mdi-clock-time-four-outline" readonly :rules="required" hide-details="auto" v-bind="attrs" v-on="on"></v-text-field>
        </template>
     ...
     <v-col lg="12">
      ...
      <v-layout d-flex>
       <v-dialog ref="dialogWeekEndsMorning" v-model="modalWeekEndMorning" :return-value.sync="data.week_end_opening" persistent width="400px">
        <template v-slot:activator="{ on, attrs }">
         <v-text-field class="" outlined v-model="data.week_end_opening" label="Opening Hour" prepend-inner-icon="mdi-clock-time-four-outline" readonly :rules="required" hide-details="auto" v-bind="attrs" v-on="on"></v-text-field>
        </template>
       ...
       <v-dialog ref="dialogWeekendsAfternoon" v-model="modalWeekEndAfternoon" :return-value.sync="data.week_end_closing" persistent width="400px">
        <template v-slot:activator="{ on, attrs }">
         <v-text-field class="" outlined v-model="data.week_end_closing" label="Closing Hour" hide-details="auto" prepend-inner-icon="mdi-clock-time-four-outline" readonly :rules="required" v-bind="attrs" v-on="on"></v-text-field>
        </template>
