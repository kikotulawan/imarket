<template>
 <div>
  ...
  <v-container class="mt-15" v-if="!initialLoading">
   <search-product />
   <v-layout class="" column>
    <div class="store-preview">
     <v-img :src="`https://imarketbe.capstone-group.online/images/banners/${selectedStore.store.banner_image}`" class="store-preview--image"></v-img>
    ...
    <v-layout class="store-preview--content">
     <div align-center justify-center class="position-relative">
      <v-avatar color="grey" size="140">
       <img v-if="selectedStore.store.logo != '' && selectedStore.store.logo != null" :src="'https://imarketbe.capstone-group.online/images/logos/' + selectedStore.store.logo" alt="Store Logo" />
...
<script>
 ...
 export default {
  ...
  methods: {
   async getStoreInfo() {
    const res = await this.$store.dispatch("market/getSelectedStoreInfo", this.$route.params.id);
    if (res.data.data == null) {
     this.$router.push("/imarket");
     this.$toast.error("Store is not yet available");
    } else {
  ...
  computed: {
   ...
   storeRating() {
    ...
    return (rating / (this.selectedStore.products.length - notCounted)).toFixed(2);
   },
  },
 };
</script>
